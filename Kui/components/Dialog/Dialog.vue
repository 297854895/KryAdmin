    ...
    <div class="k-mask-dialog-bk"></div>
      <div class="k-dialog-container">
        ...
        <div class="k-dialog-button">
          <k-button type="primary" size="small" :callBack="sure" :data="this.$data._set.button[0].data">{{this.$data._set.button[0].label}}</k-button>
          <k-button size="small" :callBack="cancel" :data="this.$data._set.button[1].data">{{this.$data._set.button[1].label}}</k-button>
        </div>
      </div>
  </div>
</template>
      ...
      sure () {
        if (this.$data._set.button) {
          if (this.$data._set.button[0].callBack){
            this.$data._set.button[0].callBack(this.$data._set.button[0].data);
          }
        }
        this.close();
      },
